...
/** LSD full interface.
 */
void LineSegmentDetection( int * n_out, double *segments,
                               ...
                               int ** reg_img, int * reg_x, int * reg_y )
{
  ...
  */
  ...
  n_seg = (int) (out->size);
  if (n_seg < *n_out) *n_out=n_seg; 

  memcpy(segments, out->values, sizeof(double)**n_out*7);
 ...
 */
void lsd_scale_region( int * n_out, double *segments,
                           ...
                           int ** reg_img, int * reg_x, int * reg_y )
{
  ...
  LineSegmentDetection( n_out, segments,img, X, Y, scale, sigma_scale, quant,
                               ang_th, log_eps, density_th, n_bins,
                               reg_img, reg_x, reg_y );
}

/*----------------------------------------------------------------------------*/
/** LSD nearly full Interface with all parameters but Region output.
 */
void lsd_with_param( int * n_out, double *segments,
                     double * img, int X, int Y, double scale, double sigma_scale, double quant, double ang_th, double log_eps, double density_th, int n_bins)
{
  LineSegmentDetection( n_out, segments,img, X, Y, scale, sigma_scale, quant,
                               ang_th, log_eps, density_th, n_bins,
                               NULL, NULL, NULL );
 ...
 */
void lsd_scale(int * n_out, double *segments,double * img, int X, int Y, double scale)
{
  lsd_scale_region(n_out,segments,img,X,Y,scale,NULL,NULL,NULL);
}

/*----------------------------------------------------------------------------*/
/** LSD Simple Interface.
 */
void lsd(int * n_out, double *segments,double * img, int X, int Y)
{
  /* LSD parameters */
  double scale = 0.8;       /* Scale the image by Gaussian filter to 'scale'. */

  lsd_scale(n_out,segments,img,X,Y,scale);
}
/*----------------------------------------------------------------------------*/
